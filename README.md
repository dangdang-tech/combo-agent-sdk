...
当前源码版本为 **0.2.0，私有预发布（UNRELEASED / PARTIAL）**。新增显式选择的 v2 付款码恢复客户端与 Host 示例，接入见[付款码恢复手册](PAYMENT_RECOVERY.md)。配套平台 [PR #368](https://github.com/dangdang-tech/Combo/pull/368) 已合入，V2 合同锁绑定其实际合并提交；这不表示已部署或完成真实支付验收。既有 v1 API 保持兼容。支付客户端、身份接入、付款后继续的示例和失败重试判断均已实现；还没有正式 Tag、Release 或 npm 发布。
