...
当前源码版本为 **0.2.0，私有预发布（UNRELEASED / PARTIAL）**。新增显式选择的 v2 付款码恢复客户端与 Host 示例，接入见[付款码恢复手册](PAYMENT_RECOVERY.md)；该候选能力依赖 Combo #366，尚未发布或部署。既有 v1 API 保持兼容。支付客户端、身份接入、付款后继续的示例和失败重试判断均已实现；还没有正式 Tag、Release 或 npm 发布。
